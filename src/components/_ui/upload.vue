...
import ScaleImgComponent from './scale-img.vue'
...
export default {
  ...
  methods: {
    ...
    onBeforeUpload (file) {
      ...
      let isInclude = this.accept.some(o => file.type.indexOf(o.replace('.', '')))
    ...
    previewUploadProgress (percent) {
      ...
      if (percent <= 50) {
        this.$refs['rightCircle'].style.cssText = `transform: rotate(${-135 + 3.6 * percent}deg)`
        this.$refs['leftCircle'].style.cssText = `transform: rotate(-135deg)`
      } else {
        this.$refs['rightCircle'].style.cssText = `transform: rotate(45deg)`
        this.$refs['leftCircle'].style.cssText = `transform: rotate(${-135 + 3.6 * (percent - 50)}deg)`
      }
    },
    ...
    onlybtnUploadProgress (percent) {
      ...
      if (percent === 100) { this.btnText = `处理中` }
    },
