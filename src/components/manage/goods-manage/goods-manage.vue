...
export default {
  ...
  methods: {
    ...
    updateGood(index, row) {
      let classType = this.tableData.class.find(
        o => o.class_name === row.class_name
      );
      this.title = "更新商品";
    ...
    submit: _.debounce(function() {
      this.$refs["goodForm"].validate(valid => {
        if (valid) {
          ...
          let url =
            this.title === "添加商品" ? ADD_GOOD_POST : UPDATE_GOOD_POST;
          this.goodForm.url_old =
            this.title === "添加商品" ? "" : this.goodForm.url_old;
          this.$http
            .post(url, this.createFormData(this.goodForm))
            .then(res => {
              if (this.vmResponseHandler(res)) {
