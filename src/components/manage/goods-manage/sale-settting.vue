...
export default {
  ...
  watch: {
    auth() {
      this.panel = this.vmHasAuth(
        this.AdminPermissionsLib.PROMOTION_ADD,
        this.auth
      )
  ...
  methods: {
    async getSalesInfo() {
      ...
      let sales = await this.$http.post(
        ADMIN_SALES_SEARCH_POST,
        this.createFormData({
          goods_ids: this.id
        })
    ...
    submit: _.debounce(async function() {
      this.$refs["goodForm"].validate(async valid => {
        ...
        if (this.vmResponseHandler(res)) {
          ...
          if (
            this.vmHasAuth(
              this.AdminPermissionsLib.PROMOTION_EDIT,
              this.auth
            ) &&
    ...
    deleteSales: _.debounce(function(row) {
      this.vmConfirm({
        ...
        confirmCallback: async () => {
          ...
          let res = await this.$http.post(ADMIN_SALES_DELETE_POST, data);

          if (this.vmResponseHandler(res)) {
            this.vmMsgSuccess("删除成功！");
            let sale = this.tableData.find(
              o => o.goods_pro_id === row.goods_pro_id
            );
