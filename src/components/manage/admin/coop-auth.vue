...
import { COOP_AUTH_USER_LIST_POST, COOP_AUTH_USER_PERMISSIONS_EDIT, COOP_AUTH_USER_PERMISSIONS_POST } from '@/lib/api.js'
...
export default {
  ...
  methods: {
    ...
    getAuthList () {
      ...
      this.$http.post(COOP_AUTH_USER_PERMISSIONS_POST, param).then(res => {
    ...
    confirm (val) {
      ...
      this.vmConfirm({
        ...
        confirmCallback: () => {
          ...
          this.$http.post(COOP_AUTH_USER_PERMISSIONS_EDIT, param).then(res => {
            loading.close()
            if (this.vmResponseHandler(res)) {
    ...
    loadData: _.debounce(function () {
      ...
      this.loading = true
      this.$http.post(COOP_AUTH_USER_LIST_POST, data).then(res => {
        if (this.vmResponseHandler(res)) {
