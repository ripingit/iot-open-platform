...
import { SET_USEREQUIPMENTMANAGEMENT_LIST_POST, GET_USEREQUIPMENTMANAGEMENT_RECORD_POST, MANAGEMENTREWORK_UPLOAD_PIC_POST, USEREQUIPMENTMANAGEMENT_ADD_POST } from '@/lib/api'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      uploadPath: MANAGEMENTREWORK_UPLOAD_PIC_POST,
      isUploading: true,
      uploadProgress: '',
  ...
  methods: {
    ...
    getEquipmentLists: _.debounce(function (currentPage) {
      ...
      this.loading = true
      this.$http.post(SET_USEREQUIPMENTMANAGEMENT_LIST_POST, data).then(res => {
    ...
    getEquipmentReworkInfo (deviceId) {
      ...
      this.$http.post(GET_USEREQUIPMENTMANAGEMENT_RECORD_POST, data).then(res => {
        if (this.vmResponseHandler(res)) {
    ...
    submit: _.debounce(function () {
      this.$refs['reworkForm'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post(USEREQUIPMENTMANAGEMENT_ADD_POST, this.createFormData(this.reworkForm)).then(res => {
            if (this.vmResponseHandler(res)) {
