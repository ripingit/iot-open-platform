<template>
  <div class="content-container">
          ...
          <el-row>
            <el-col :span="24">
              <el-table v-loading="loading" :data="tableData.data" style="width: 100%">
                ...
                <el-table-column prop="client_id" label="ID"></el-table-column>
                <el-table-column prop="client_secret" label="KEY"></el-table-column>
                <el-table-column prop="create_time" width="150" label="生成时间"></el-table-column>
                <el-table-column prop="app_name" width="150" label="绑定App"></el-table-column>
                <el-table-column prop="notify_url" label="回调地址">
                  <template slot-scope="scope">
                    <a
                      class="wait"
                      href="javascript:void(0)"
                      v-if="scope.row.notify_url"
                      @click="setNotifyUrl(scope.row)"
                    >{{ scope.row.notify_url }}</a>
                    <a
                      class="success"
                      href="javascript:void(0)"
                      v-else
                ...
                <el-table-column
                  label="操作"
                  width="100"
                  v-if="vmHasAuth(CoopPermissionsLib.ADD_NOTIFY_URL, tableData.res)"
                >
                  <template slot-scope="scope">
                    <el-button
                      class="btn-circle"
    ...
    <el-dialog title="设置回调" :visible.sync="isToSetNotifyUrl" width="50rem" center>
    ...
    <el-dialog title="创建KEY" :visible.sync="createKeyDialog" width="50rem" center>
      <el-form
        label-width="100px"
        status-icon
...
export default {
  ...
  data() {
    ...
    return {
        ...
        columns: [
          {
            ...
            label: "协议语言",
            render: value =>
              this.protocolLangTemplate.find(o => o.id === value).name
          },
          {
            prop: "type",
            label: "协议类型",
      ...
      rules: {
        client_id: [{ validator: validateIsEmpty, trigger: "change" }],
        notify_url: [{ validator: validateIsEmpty, trigger: "blur" }]
      },
      keyFormRules: {
        app_name: [{ validator: validateIsEmpty, trigger: "blur" }]
      },
      smsFormRules: {
  ...
  methods: {
    ...
    submitNotifyUrl: _.debounce(function() {
      this.$refs["notifyForm"].validate(valid => {
        if (valid) {
          ...
          let loading = this.vmLoadingFull();
          this.$http
            .post(SET_NOTIFY_URL_POST, param)
            .then(res => {
              if (this.vmResponseHandler(res)) {
                this.vmMsgSuccess("设置成功");
    ...
    generateKey: _.debounce(function() {
      this.$refs["keyForm"].validate(valid => {
        if (valid) {
          ...
          let loading = this.vmLoadingFull();
          this.$http
            .post(GENERATE_KEY_ID_POST, param)
            .then(res => {
              if (this.vmResponseHandler(res)) {
    ...
    getEmialLists: _.debounce(function(page) {
      ...
      this.$http
        .post(COOP_EMAIL_QUERY_POST, data)
        .then(res => {
          if (this.vmResponseHandler(res)) {
            this.smsTableData = res.data.data;
    ...
    getProtocolLists: _.debounce(function(page) {
      ...
      this.$http
        .post(COOP_PROTOCOL_QUERY_POST, data)
        .then(res => {
          if (this.vmResponseHandler(res)) {
            this.smsTableData = res.data.data;
